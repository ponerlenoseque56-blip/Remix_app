...
class MainMobileActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // --- BOTON CERRAR SESIÓN (CRUZ) ---
        binding.btnLogout.setOnClickListener {
            androidx.appcompat.app.AlertDialog.Builder(this)
                .setTitle("Cerrar sesión")
                .setMessage("¿Querés cerrar la sesión y volver al login?")
                .setPositiveButton("Sí, salir") { _, _ ->
                    getSharedPreferences("remix_panel", MODE_PRIVATE).edit().clear().apply()
    ...
    override fun onDestroy() { dismissUpdateDialog(); _binding = null; super.onDestroy() }
    ...
    private fun showUpdateDialog(state: MainViewModel.State.SuccessCheckingUpdate) {
        ...
        updateAppDialog = UpdateAppMobileDialog(this, state.newReleases).also { dialog ->
            dialog.setOnUpdateClickListener { if (!dialog.isLoading) { viewModel.downloadUpdate(this@MainMobileActivity, state.asset) } }
            dialog.show()
        }
    }
    ...
    private fun isTopLevelProviderDestination(destinationId: Int?): Boolean {
        return destinationId in setOf(R.id.search, R.id.home, R.id.movies, R.id.tv_shows, R.id.settings)
    }
    ...
    private suspend fun requestResolverPayload(wsUrl: String, token: String): ResolverPayload? = withContext(Dispatchers.IO) {
            ...
            suspendCancellableCoroutine { continuation ->
                ...
                val socket = resolverWebSocketClient.newWebSocket(request, object : WebSocketListener() {
                    override fun onOpen(webSocket: WebSocket, response: Response) {
                        Log.d("ResolverWS", "Connected -> requesting URL"); webSocket.send("resolve:$token")
                    }
                        ...
                        when {
                            text.startsWith("payload:") -> {
                                ...
                                val parsed = runCatching {
                                    ...
                                    ResolverPayload(url = json.optString("url"))
                                }.getOrNull()
                    ...
                    override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                        if (!continuation.isActive) { Log.d("ResolverWS", "WS resolve cancelled or timed out"); return }
                        Log.e("ResolverWS", "WS resolve failed", t); continuation.resume(null)
                    }
                })
                ...
                suspendCancellableCoroutine { continuation ->
                    ...
                    val socket = resolverWebSocketClient.newWebSocket(request, object : WebSocketListener() {
                        override fun onOpen(webSocket: WebSocket, response: Response) {
                            ...
                            val encodedCookies = cookies?.takeIf { it.isNotBlank() }?.let { Base64.getEncoder().encodeToString(it.toByteArray(Charsets.UTF_8)) }
                            val message = if (encodedCookies.isNullOrBlank()) { "done:$token" } else { "done:$token:$encodedCookies" }
                            webSocket.send(message)
                        }
                        override fun onMessage(webSocket: WebSocket, text: String) {
                            if (text == "ack:$token" && continuation.isActive) { continuation.resume(Unit); webSocket.close(1000, null) }
                        }
                        override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                            if (!continuation.isActive) { Log.d("ResolverWS", "WS done cancelled or timed out"); return }
                            Log.e("ResolverWS", "WS failed", t); continuation.resume(Unit)
                        }
                    })
                    continuation.invokeOnCancellation { socket.cancel() }
                }
    ...
    private fun showResolverConnectionErrorDialog(ws: String, token: String) {
        if (isFinishing || isDestroyed) return
        androidx.appcompat.app.AlertDialog.Builder(this).setTitle(R.string.app_name).setMessage("Unable to reach the TV bypass websocket. Retry?").setPositiveButton("Retry") { _, _ -> resolve(ws, token) }.setNegativeButton(android.R.string.cancel) { _, _ -> clearResolverState() }.setOnCancelListener { clearResolverState() }.show()
    }
    private fun showPostBypassCloseDialog() {
        if (isFinishing || isDestroyed) return
        androidx.appcompat.app.AlertDialog.Builder(this).setTitle(R.string.app_name).setMessage("Bypass completed. Do you want to close the app?").setPositiveButton("Close app") { _, _ -> closeTask() }.setNegativeButton("Keep open", null).setOnCancelListener(null).show()
    }
